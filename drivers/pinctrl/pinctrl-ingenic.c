...
static int ingenic_gpio_request(struct gpio_chip *chip, unsigned offset)
{
	...
	ingenic_gpio_set_pull(jzgc, BIT(offset), false);
...
{
#if defined(PINCTL_CFG_FILTER)
	if ((PINCTL_CFG_TYPE_MSK & cfgval) == PINCTL_CFG_FILTER) {
		int value = (cfgval >> PINCTL_CFGVAL_SFT) & PINCTL_FILTER_MASK;
...
static const struct pinctrl_ops ingenic_pctl_ops = {
	...
	.dt_node_to_map         = ingenic_dt_node_to_map,
	...
	for(i = 0; i < num_configs; i++) {
		...
		switch (param) {
			case PIN_CONFIG_BIAS_PULL_PIN_DEFAULT:
				if (!(jzgc->pull_bitmap & BIT(pin)))
					return -EINVAL;
				ingenic_gpio_set_pull(jzgc, BIT(pin), true);
				break;
			case PIN_CONFIG_BIAS_DISABLE:
				if ((!(jzgc->pull_bitmap & BIT(pin))))
					return -EINVAL;
				ingenic_gpio_set_pull(jzgc, BIT(pin), false);
				break;
		...
		char *buf)
{
	...
	len += snprintf(buf+len, PAGE_SIZE - len, "\n");
	for (idx = 0;  idx < pctl->num_chips; idx++)
		len += snprintf(buf + len, PAGE_SIZE - len, "PULL:|%08x|",
...
{
	const struct ingenic_priv *priv = jzgc->pctl->priv;
	u32 pm_pull, pm_high, pm_low, pm_nopull;
	...
	if (pm_pull) {
		...
		ingenic_gpio_set_pull(jzgc, pm_pull, true);
	}
	if (pm_nopull) {
		ingenic_gpio_set_func(jzgc, priv->have_shadow, GPIO_INPUT, pm_nopull);
		ingenic_gpio_set_pull(jzgc, pm_nopull, false);
	}
	if (pm_high)
		ingenic_gpio_set_func(jzgc, priv->have_shadow, GPIO_OUTPUT1, pm_high);
	if (pm_low)
		ingenic_gpio_set_func(jzgc, priv->have_shadow, GPIO_OUTPUT0, pm_high);
...
static const struct of_device_id ingenic_pinctrl_dt_match[] = {
	...
	{ .compatible = "ingenic,x2000-pinctrl", .data = (void *)common_priv_data},
	{},
};
