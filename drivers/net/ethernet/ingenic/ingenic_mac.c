...
__attribute__((__unused__)) static void ingenic_mac_dump_dma_regs(const char *func, int line)
{
...
__attribute__((__unused__)) static void ingenic_mac_dump_mac_regs(const char *func, int line)
{
...
__attribute__((__unused__)) static void ingenic_mac_dump_all_regs(const char *func, int line) {
	ingenic_mac_dump_dma_regs(func, line);
	ingenic_mac_dump_mac_regs(func, line);
}
...
#if defined(CONFIG_INGENIC_MAC_RMII)
	phydev = phy_connect(dev, dev_name(&phydev->dev), &ingenic_mac_adjust_link,
			 PHY_INTERFACE_MODE_RMII);
#elif defined(CONFIG_INGENIC_MAC_RGMII)
	phydev = phy_connect(dev, dev_name(&phydev->dev), &ingenic_mac_adjust_link,
			 PHY_INTERFACE_MODE_RGMII);
#elif defined(CONFIG_INGENIC_MAC_GMII)
	phydev = phy_connect(dev, dev_name(&phydev->dev), &ingenic_mac_adjust_link,
...
static void ingenic_mac_init(void) {
	/*
...
static int ingenic_mac_probe(struct platform_device *pdev)
{
	...
	lp->clk_gate = devm_clk_get(&pdev->dev, "gate_mac");
	lp->clk_cgu = devm_clk_get(&pdev->dev, "cgu_mac");
	if (IS_ERR(lp->clk_gate) || IS_ERR(lp->clk_cgu)) {
		rc =  -ENODEV;
		goto err_out_free_netdev;
	}

	if (clk_set_rate(lp->clk_cgu, 50000000) || (clk_prepare_enable(lp->clk_cgu))) {
		rc = -ENODEV;
		goto err_out_clk_gate_disable;
	}

	if (clk_prepare_enable(lp->clk_gate)) {
		rc = -ENODEV;
		goto err_out_free_netdev;
	}
