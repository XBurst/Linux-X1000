...
static inline void tcu_disable_counter(int id)
{
...
static inline void tcu_start_clock(int id)
{
	tcu_writel(tcu, TCU_TSCR, BIT(id));
}
static inline void tcu_stop_clock(int id)
{
	tcu_writel(tcu, TCU_TSSR, BIT(id));
}
static inline void tcu_set_counter(int id, unsigned int val)
{
	tcu_writel(tcu, CH_TCNT(id), val);
}
static inline int tcu_get_counter(int id)
{
...
static inline void tcu_disable_all_channel(void)
{
	...
	tcu_writel(tcu, TCU_TSSR, 0x000100ff);
}
...
void ingenic_tcu_set_full_num(int id, uint16_t full_num)
{
	tcu_writel(tcu, CH_TDFR(id), full_num);
}
EXPORT_SYMBOL_GPL(ingenic_tcu_set_full_num);
void ingenic_tcu_set_half_num(int id, uint16_t half_num)
{
	tcu_writel(tcu, CH_TDHR(id), half_num);
}
EXPORT_SYMBOL_GPL(ingenic_tcu_set_half_num);
...
static int init_mfd_cells(struct ingenic_tcu *tcu, struct mfd_cell *cells)
{
	...
	for_each_child_of_node(tcu->np, child) {
		...
		tcu->idmap[id] = i;
		tcu->channel_mask |= BIT(id);
		chn->enable = tcu_start_clock;
		chn->disable = tcu_stop_clock;
		cells[i].id = id;
		cells[i].name = tmp;
