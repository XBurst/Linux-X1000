...
struct ingenic_nand_base_param {
	uint32_t pagesize;
	uint32_t blocksize;
	uint32_t oobsize;
	uint32_t flashsize;
...
struct ingenic_nand_partition_param {
...
struct device_id_struct {
	uint8_t id_device;
	char *name;
	struct ingenic_nand_base_param *param;
};
...
struct ingenic_nand_erase {
	void (*write_enable)(struct sfc_transfer *, struct cmd_info *, struct flash_operation_message *);
	void (*block_erase)(struct sfc_transfer *, struct cmd_info *, struct flash_operation_message *);
	int32_t (*get_feature)(struct flash_operation_message *);
};

struct ingenic_nand_ops {
	struct ingenic_nand_read nand_read_ops;
	struct ingenic_nand_write nand_write_ops;
	struct ingenic_nand_erase nand_erase_ops;
};

struct ingenic_nand_device {
	uint8_t id_manufactory;
	struct device_id_struct *id_device_list;
	uint8_t id_device_count;

	struct ingenic_nand_ops ops;
	struct list_head nand;
};

struct ingenic_nand_descriptor {
	uint8_t id_manufactory;
	uint8_t id_device;

	struct ingenic_nand_base_param param;
	struct ingenic_nand_partition_param partition;
	struct ingenic_nand_ops *ops;
};

struct ingenic_spinand_partition {
...
struct ingenic_sfc_nand_param {
	char name[32];
	short nand_id;
	struct ingenic_nand_base_param param;
};

struct ingenic_sfc_nand_burner_param {
	unsigned int magic_num;
	int partition_num;
	struct ingenic_spinand_partition *partition;
};
