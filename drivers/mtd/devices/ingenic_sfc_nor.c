...
static struct sfc_flash *to_ingenic_spi_norflash(struct mtd_info *mtd_info)
{
...
static void get_current_operate_cmd(struct sfc_flash *flash)
{
...
static int ingenic_spi_norflash_read_params(struct sfc_flash *flash, loff_t from, size_t len, unsigned char *buf)
{
	struct sfc_transfer transfer;
	struct sfc_message message;
	struct cmd_info cmd;
...
static int ingenic_spi_norflash_match_device(struct sfc_flash *flash)
{
	int ret = 0;

	flash->params = (struct burner_params*)devm_kzalloc(flash->dev, sizeof(struct burner_params),GFP_KERNEL);
	if(flash->params == NULL){
		dev_err(flash->dev, "%s: devm_kzalloc params mem error !\n", __func__);
		return -ENOMEM;
	}

	ret = ingenic_spi_norflash_read_params(flash, SPIFLASH_PARAMER_OFFSET,
								sizeof(struct burner_params), (unsigned char *)flash->params);
	if (ret) {
		dev_err(flash->dev, "get nor flash params from burner failed !\n");
	}
	...
	if (flash->params->magic == NOR_MAGIC) {
		...
		flash->g_nor_info = NULL;
		dev_err(flash->dev, "warning : can not read params from flash ! \n");
	}
#ifdef DEBUG_CLONER_PARAMS
...
static int __init ingenic_sfc_probe(struct platform_device *pdev)
{
	...
	flash = devm_kzalloc(&pdev->dev, sizeof(struct sfc_flash), GFP_KERNEL);
	if (!flash) {
		dev_err(&pdev->dev, "ERROR: %s %d devm_kzalloc() error !\n",__func__,__LINE__);
		return -ENOMEM;
	}

	flash->sfc = devm_kzalloc(&pdev->dev, sizeof(struct sfc), GFP_KERNEL);
	if (!flash->sfc) {
		dev_err(&pdev->dev, "ERROR: %s %d kzalloc() error !\n",__func__,__LINE__);
		return -ENOMEM;
	}
	...
	if (ret) {
		...
		dev_err(&pdev->dev, "ERROR: %s %d sfc init error !\n",__func__,__LINE__);
		return -ENOENT;
	}
	...
	ret = ingenic_spi_norflash_match_device(flash);
	if (ret) {
		dev_err(&pdev->dev, "get spi flash info from burner failed !\n");
		ret =  -ENODEV;
		goto err_no_pdata;
	}
	...
	flash->mtd.erasesize    = spi_nor_info->erase_size;
	...
	ingenic_mtd_partition = (struct mtd_partition*)devm_kzalloc(flash->dev, sizeof(struct mtd_partition) * num_partition_info,GFP_KERNEL);
	if (!ingenic_mtd_partition) {
		dev_err(flash->dev, "malloc partitions mem error !\n");
		ret = -ENOMEM;
		goto err_no_pdata;
	}
	...
	for (i = 0; i < num_partition_info; i++) {
		...
		ingenic_mtd_partition[i].name = flash->norflash_partitions->nor_partition[i].name;
		ingenic_mtd_partition[i].offset = flash->norflash_partitions->nor_partition[i].offset;

		if (flash->norflash_partitions->nor_partition[i].size == -1) {
			ingenic_mtd_partition[i].size = MTDPART_SIZ_FULL;
		} else {
			ingenic_mtd_partition[i].size = flash->norflash_partitions->nor_partition[i].size;
		}
	...
	if (flash->quad_mode) {
		if (flash->nor_flash_ops->set_quad_mode) {
			...
			if (ret < 0) {
				...
				dev_err(&pdev->dev, "set quad mode error !\n");
			} else {
				flash->quad_succeed = 1;
				dev_err(&pdev->dev, "nor flash quad mode is set, now use quad mode!\n");
			}
		}
	}
	...
	if (ret) {
		dev_err(flash->dev, "mtd device parse failed\n");
		goto err_no_pdata;
	}
...
err_no_pdata:
	clk_disable_unprepare(flash->sfc->clk_gate);
	clk_disable_unprepare(flash->sfc->clk);
	devm_kfree(&pdev->dev, flash->sfc);
	devm_kfree(&pdev->dev, flash);
	return ret;
}
