...
{
	struct ingenic_nand_descriptor *nand_desc = flash->flash_info;
	struct ingenic_nand_base_param *param = &nand_desc->param;
...
{
	struct ingenic_nand_descriptor *nand_desc = flash->flash_info;
	struct ingenic_nand_read *nand_read_ops = &nand_desc->ops->nand_read_ops;
...
static int32_t ingenic_sfcnand_read_oob(struct mtd_info *mtd, loff_t from, struct mtd_oob_ops *ops)
{
	...
	if(ops->datbuf) {
		ret = ingenic_sfc_nand_read(flash, pageaddr, 0, ops->datbuf, mtd->writesize);
		if(ret < 0) {
			dev_err(flash->dev, "spi nand read error %s %s %d, ret = %d\n",__FILE__,__func__,__LINE__, ret);
			if(ret == -EIO) {
	...
	if(ret < 0)
		dev_err(flash->dev, "spi nand read error %s %s %d \n",__FILE__,__func__,__LINE__);
		if(ret != -EIO)
...
{
	struct ingenic_nand_descriptor *nand_desc = flash->flash_info;
	struct ingenic_nand_write *nand_write_ops = &nand_desc->ops->nand_write_ops;
...
{
	struct ingenic_nand_descriptor *nand_desc = flash->flash_info;
	struct ingenic_nand_erase *nand_erase_ops = &nand_desc->ops->nand_erase_ops;
...
static int ingenic_sfc_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	struct sfc_flash *flash = to_ingenic_spi_nand(mtd);
...
static int ingenic_sfcnand_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	struct nand_chip *chip = mtd->priv;
	int ret;
	ret = ingenic_sfcnand_block_isbab(mtd, ofs);
	if (ret) {
...
static int ingenic_sfcnand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if((ret = ingenic_sfc_nand_erase(mtd, instr))) {
		dev_err(flash->dev, "WARNING: block %d erase fail !\n",(uint32_t)instr->addr / mtd->erasesize);
		if((ret = ingenic_sfcnand_block_markbad(mtd, instr->addr))) {
			dev_info(flash->dev, "mark bad block error, there will occur error,so exit !\n");
			return -1;
		}
	}
	instr->state = MTD_ERASE_DONE;
...
static int ingenic_sfcnand_read(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
	...
	while(len) {
		...
		if(ret < 0) {
			...
			dev_warn(flash->dev, "%s %s %d: ingenic_sfc_nand_read, ecc value = %d\n",
						__FILE__, __func__, __LINE__, ret);
			ret_eccvalue = ret;
		}
...
static int ingenic_sfcnand_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
	...
	while(len) {
		...
		if((ret = ingenic_sfc_nand_write(flash, (u_char *)buf, pageaddr, columnaddr, wlen))) {
			dev_err(flash->dev, "spi nand write fail %s %s %d\n",__FILE__,__func__,__LINE__);
			break;
		}
		*retlen += wlen;
		len -= wlen;
...
static int ingenic_sfc_nand_set_feature(struct sfc_flash *flash, uint8_t addr, uint8_t val)
{
	struct sfc_transfer transfer;
	struct sfc_message message;
	struct cmd_info cmd;
...
{
	struct ingenic_nand_descriptor *nand_desc = flash->flash_info;
	struct ingenic_nand_device *nand_device;
	struct sfc_transfer transfer;
	struct sfc_message message;
	struct cmd_info cmd;
...
static int32_t __init nand_partition_param_copy(struct sfc_flash *flash, struct ingenic_sfc_nand_burner_param *burn_param) {
	struct ingenic_nand_descriptor *nand_desc = flash->flash_info;
	int i = 0, count = 5, ret;
	size_t retlen = 0;
	...
	burn_param->partition = kzalloc(nand_desc->partition.num_partition * sizeof(struct ingenic_spinand_partition), GFP_KERNEL);
...
partition_retry_read:
	ret = ingenic_sfcnand_read(&flash->mtd, SPIFLASH_PARAMER_OFFSET + sizeof(*burn_param) - sizeof(burn_param->partition),
		nand_desc->partition.num_partition * sizeof(struct ingenic_spinand_partition),
...
static struct ingenic_sfc_nand_burner_param *burn_param;
static int32_t __init ingenic_nand_partition(struct sfc_flash *flash) {

	int32_t ret = 0, retlen = 0, count = 5;

	burn_param = kzalloc(sizeof(struct ingenic_sfc_nand_burner_param), GFP_KERNEL);
	if(!burn_param) {
...
param_retry_read:
	ret = ingenic_sfcnand_read(&flash->mtd, SPIFLASH_PARAMER_OFFSET,
		sizeof(struct ingenic_sfc_nand_burner_param), &retlen, (u_char *)burn_param);
	if((ret < 0) && count--)
...
int ingenic_nand_register(struct ingenic_nand_device *flash) {
	list_add_tail(&flash->nand, &nand_list);
	return 0;
}
EXPORT_SYMBOL_GPL(ingenic_nand_register);
...
static int __init ingenic_sfc_probe(struct platform_device *pdev)
{
	...
	struct ingenic_nand_descriptor *nand_desc;
	int ret;
	...
	flash->sfc = kzalloc(sizeof(struct sfc), GFP_KERNEL);
	if(!flash->sfc){
		return -ENOMEM;
	}
	...
	nand_desc = kzalloc(sizeof(struct ingenic_nand_descriptor), GFP_KERNEL);
	if(!nand_desc) {
		kfree(flash);
	    	kfree(chip);
		return -ENOMEM;
...
#define TSHSL_W	    50
	...
	chip->buffers->databuf = kzalloc(nand_desc->param.pagesize + nand_desc->param.oobsize, GFP_KERNEL);
	...
	if((ret = ingenic_nand_partition(flash))) {
		if(ret == -EINVAL)
			return 0;
...
/*	dump_flash_info(flash);*/
	ret = mtd_device_parse_register(&flash->mtd, ingenic_probe_types, NULL, nand_desc->partition.partition, nand_desc->partition.num_partition);
	if (ret) {
		kfree(nand_desc->partition.partition);
		kfree(burn_param->partition);
		kfree(burn_param);
