...
static int ingenic_jpeg_open(struct file *file)
{
	...
	struct ingenic_jpeg_ctx *ctx;
	struct ingenic_jpeg_fmt *out_fmt;
	int ret = 0;

	ctx = devm_kzalloc(jpeg->dev, sizeof(*ctx), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;
...
static unsigned int ingenic_jpeg_poll(struct file *file,
...
static int ingenic_jpeg_mmap(struct file *file, struct vm_area_struct *vma)
{
	struct ingenic_jpeg *jpeg = video_drvdata(file);
	struct ingenic_jpeg_ctx *ctx = fh_to_ctx(file->private_data);
	int ret;
...
static const struct v4l2_file_operations ingenic_jpeg_fops = {
	...
	.poll		= ingenic_jpeg_poll,
	.unlocked_ioctl	= video_ioctl2,
	.mmap		= ingenic_jpeg_mmap,
};
...
static int ingenic_jpeg_reqbufs(struct file *file, void *priv,
...
static int ingenic_jpeg_querybuf(struct file *file, void *priv,
...
static int ingenic_jpeg_qbuf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	struct ingenic_jpeg_ctx *ctx = fh_to_ctx(priv);
...
static int ingenic_jpeg_dqbuf(struct file *file, void *priv,
							  struct v4l2_buffer *buf)
{
	struct ingenic_jpeg_ctx *ctx = fh_to_ctx(priv);
...
static const struct v4l2_ioctl_ops ingenic_jpeg_ioctl_ops = {
	...
	.vidioc_reqbufs			= ingenic_jpeg_reqbufs,
	.vidioc_querybuf		= ingenic_jpeg_querybuf,

	.vidioc_qbuf			= ingenic_jpeg_qbuf,
	.vidioc_dqbuf			= ingenic_jpeg_dqbuf,
...
static void ingenic_jpeg_lock(void *prv)
{
	struct ingenic_jpeg_ctx *ctx = prv;
	struct ingenic_jpeg *jpeg = ctx->jpeg;
	mutex_lock(&jpeg->lock);
}

static void ingenic_jpeg_unlock(void *prv)
{
	struct ingenic_jpeg_ctx *ctx = prv;
	struct ingenic_jpeg *jpeg = ctx->jpeg;
	mutex_unlock(&jpeg->lock);
...
static struct v4l2_m2m_ops ingenic_jpeg_m2m_ops = {
	...
	.lock		= ingenic_jpeg_lock,
	.unlock		= ingenic_jpeg_unlock,
};
